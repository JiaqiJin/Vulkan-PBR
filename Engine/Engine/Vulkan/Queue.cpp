...
namespace Vulkan
{
	std::shared_ptr<UniqueQueue> UniqueQueue::Create(const std::shared_ptr<Device>& device, uint32_t family_index, uint32_t queue_index)
	{
	...
	VkResult Queue::WaitIdle() const
	{
		...
		return vkQueueWaitIdle(m_unique_queue->m_queue);
	}
