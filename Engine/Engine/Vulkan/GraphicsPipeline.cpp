...
namespace Vulkan
{
	GraphicsPipeline::GraphicsPipeline(const std::shared_ptr<PipelineLayout>& pipeline_layout,
		const std::shared_ptr<RenderPass>& render_pass, const VkGraphicsPipelineCreateInfo& create_info)
		: PipelineBase(pipeline_layout), m_render_pass(render_pass)
	{
		...
		if (vkCreateGraphicsPipelines(pipeline_layout->GetDevicePtr()->GetHandle(),
			pipeline_layout->GetDevicePtr()->GetPipelineCacheHandle(), 1, &new_info, nullptr, &m_pipeline) != VK_SUCCESS)
			K_ERROR("Failed to Create GraphicsPipeline");
	}
		...
		: PipelineBase(pipeline_layout), m_render_pass(render_pass)
	{
		...
		// Set Graphic Pipeline Info
		pipeline_state.SetGraphicsPipelineCreateInfo(&create_info);
		create_info.subpass = subpass;

		if (vkCreateGraphicsPipelines(pipeline_layout->GetDevicePtr()->GetHandle(),
			pipeline_layout->GetDevicePtr()->GetPipelineCacheHandle(), 1, &create_info, nullptr, &m_pipeline) != VK_SUCCESS)
			K_ERROR("Failed to Create GraphicsPipeline");
	}
	...
	{
		info->pRasterizationState = &m_rasterization_state.m_create_info; // Rasterization
		info->pVertexInputState = m_vertex_input_state.m_enable ? &m_vertex_input_state.m_create_info : nullptr; // Vertex Input
		info->pInputAssemblyState = m_input_assembly_state.m_enable ? &m_input_assembly_state.m_create_info : nullptr; // Input Asssambly
		info->pTessellationState = m_tessellation_state.m_enable ? &m_tessellation_state.m_create_info : nullptr; // Tessellation
		info->pViewportState = m_viewport_state.m_enable ? &m_viewport_state.m_create_info : nullptr; // View port
		info->pMultisampleState = m_multisample_state.m_enable ? &m_multisample_state.m_create_info : nullptr; // MultiSample
		info->pDepthStencilState = m_depth_stencil_state.m_enable ? &m_depth_stencil_state.m_create_info : nullptr; // Depth Stencil
		info->pColorBlendState = m_color_blend_state.m_enable ? &m_color_blend_state.m_create_info : nullptr; // Color Blend
		info->pDynamicState = m_dynamic_state.m_enable ? &m_dynamic_state.m_create_info : nullptr; // Dynamic State
	}

	void GraphicsPipelineState::RasterizationState::Initialize(VkPolygonMode polygon_mode, VkFrontFace front_face, VkCullModeFlags cull_mode) 
	{
	...
	void GraphicsPipelineState::ColorBlendState::Enable(const std::vector<VkPipelineColorBlendAttachmentState>& color_blend_attachments) 
	{
		m_enable = true;
