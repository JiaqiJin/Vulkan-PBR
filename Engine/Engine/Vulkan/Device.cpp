...
namespace Vulkan
{
	std::shared_ptr<Device> Device::Create(const DeviceCreateInfo& device_create_info, void* p_next)
	{
		...
		ret->m_physical_device = device_create_info.m_physical_device_ptr;
		...
		const std::vector<const char*>& extensions, void* p_next)
	{
		...
		create_info.pEnabledFeatures = &m_physical_device->GetFeatures();
	...
	VkResult Device::create_allocator()
	{
		...
		create_info.physicalDevice = m_physical_device->GetHandle();
