...
VulkanTexture* ResourceManager::loadHDRTexture(int id, const char* path)
{
	auto it = textures.find(id);
	if (it != textures.end())
	{
		std::cerr << "ResourceManager::loadHDRTexture(): " << id << " is already taken by another texture" << std::endl;
		return nullptr;
	}
