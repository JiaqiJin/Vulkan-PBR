...
namespace RHI
{
	...
	void Renderer::initEnvironment(const UniformBufferObject* ubo, const RenderScene* scene)
	{
		...
		setEnvironment(scene, ubo->currentEnvironment);
	}
	...
	void Renderer::render(const UniformBufferObject* ubo, const RenderScene* scene, const VulkanRenderFrame& frame)
	{
	...
	void Renderer::update(UniformBufferObject* ubo, const RenderScene* scene)
	{
		...
		if (currentEnvironment != ubo->currentEnvironment)
		{
			currentEnvironment = ubo->currentEnvironment;
			setEnvironment(scene, ubo->currentEnvironment);
		}
	}
